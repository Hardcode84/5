...
// RUN:   | hc-opt --hc-front-fold-region-defs --hc-front-inline --convert-hc-front-to-hc --hc-promote-names \
// RUN:   | hc-opt \
...
// CHECK-SAME: (%[[GROUP:arg[0-9]+]]: !hc.undef, %[[A:arg[0-9]+]]: !hc.undef, %[[B:arg[0-9]+]]: !hc.undef, %[[C:arg[0-9]+]]: !hc.undef)
...
// CHECK: %[[AK:[^ ]+]] = hc.buffer_dim %[[A]], axis = 1 : !hc.undef -> !hc.undef
...
// CHECK: %[[A_COL:[^ ]+]] = hc.slice_expr
// CHECK: hc.load %[[A]][%[[A_ROW]], %[[A_COL]]] {shape = #hc.shape<["16", "16"]>} : (!hc.undef, !hc.undef, !hc.undef) -> !hc.undef
// CHECK: %[[B_ROW:[^ ]+]] = hc.slice_expr
// CHECK: %[[B_COL:[^ ]+]] = hc.slice_expr
// CHECK: hc.load %[[B]][%[[B_ROW]], %[[B_COL]]] {shape = #hc.shape<["16", "16"]>} : (!hc.undef, !hc.undef, !hc.undef) -> !hc.undef
...
// CHECK: hc.call @store_wmma_tile(%[[GROUP]], %[[C]], %[[ROW0]], %[[COL0]], %[[ACC_FINAL]]#2) : (!hc.undef, !hc.undef, !hc.undef, !hc.undef, !hc.undef) -> !hc.undef
