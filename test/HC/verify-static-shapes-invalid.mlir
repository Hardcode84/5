...
// rank. `LayoutAttr` pins `index_syms.size() == shape_syms.size()`, so
// the source rank is the canonical bind count whether or not a layout
// is attached.
hc.func @too_many_indices(%buf: !hc.buffer<f32, ["M"]>,
                          %i: !hc.idx<"0">) {
  %four = hc.const<4 : i64> : !hc.idx<"4">
  %shape = hc.tuple(%four) : (!hc.idx<"4">) -> tuple<!hc.idx<"4">>
  // expected-error @+1 {{has 2 index operand(s) for source rank 1}}
