...
//
// RUN: hc-opt %s --hc-lower-launch-scalar-ops --hc-lower-launch-shaped-constants --hc-lower-launch-memory-access --hc-lower-launch-body --hc-reconcile-generic-operands --reconcile-unrealized-casts | FileCheck %s
...
module {
  ...
  // which dispatches one slot per thread; cross-generic synchronization
  // on the populated tile lives on `hc-insert-workgroup-barriers`,
  // which runs in between and owns every `gpu.barrier` on workgroup-AS
  // storage. Every workgroup-AS write stays inside a structured generic
  // so the barrier pass has a uniform surface to work on.
  ...
  func.func @workitem_float_arith_body(%dst: !hc.ptr<global, f32>) {
    ...
    gpu.launch blocks(%bx, %by, %bz) in (%gxr = %c1, %gyr = %c1, %gzr = %c1)
      ...
      hc.generic
          iter (parallel i = %eight : !hc.idx<"8">)
          ins (%tile at [#hc.expr<"i">] : !hc.bare_tensor<f32, ["8"]>,
               %tile at [#hc.expr<"i">] : !hc.bare_tensor<f32, ["8"]>)
          outs (%dst at [#hc.expr<"i">] : !hc.ptr<global, f32>)
          -> () {
