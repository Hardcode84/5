...
// Address-space keyword <-> enum. The enum is dialect-private (no
// specialized attr companion); these helpers are the only round-trip
// surface, so a typo in surface IR ("workgrupe") fails at parse with a
// listed-alternatives diagnostic instead of being silently coerced.
static FailureOr<AddrSpace> parseAddrSpaceKeyword(AsmParser &parser) {
  llvm::SMLoc loc = parser.getCurrentLocation();
  StringRef kw;
  if (parser.parseKeyword(&kw))
    return failure();
  std::optional<AddrSpace> kind = symbolizeAddrSpace(kw);
  if (!kind) {
    parser.emitError(loc)
        << "expected `workgroup`, `global`, or `private`, got '" << kw << "'";
    return failure();
  }
  return *kind;
}
...
Type PtrType::parse(AsmParser &parser) {
  ...
  Type elementType;
  bool sawAddrSpace = false;
  AddrSpace addrSpace = AddrSpace::Workgroup;

  // Optional positional element type. `parseOptionalKeyword(StringRef)`
  // matches only the exact keyword, so it works as a peek for the
  // opaque-pointer form (`addrspace = ...` immediately after `<`)
  // without consuming a bare element-type identifier like `f16`.
  if (succeeded(parser.parseOptionalKeyword("addrspace"))) {
    if (parser.parseEqual())
      return {};
    FailureOr<AddrSpace> parsed = parseAddrSpaceKeyword(parser);
    if (failed(parsed))
      return {};
    addrSpace = *parsed;
    sawAddrSpace = true;
  ...
  // After the optional element, `, addrspace = <kw>` is required when we
  // didn't already consume the keyword above.
  while (!sawAddrSpace) {
    ...
    if (name == "addrspace") {
      ...
      parser.emitError(nameLoc) << "unknown !hc.ptr field '" << name << "'";
      return {};
    }
  }
  ...
  if (parser.parseGreater())
    return {};
  return PtrType::get(ctx, elementType, addrSpace);
}

void PtrType::print(AsmPrinter &printer) const {
  printer << "<";
  if (Type elem = getElementType())
    printer << elem << ", ";
  printer << "addrspace = " << stringifyAddrSpace(getAddrSpace()) << ">";
}
